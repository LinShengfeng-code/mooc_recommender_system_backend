...
# @Desc: {}
# @Date: 2022/2/26 3:30 下午
import torch
import numpy as np
from .DataProcessing import Construct_DataLoader
...
class Trainer(object):
    ...
    def _train_single_batch(self, batch_x, batch_mask_x):
        ...
        loss, rmse = self._model.loss(res=ratings_pred, input_score=batch_x, mask=batch_mask_x, optimizer=self._optimizer)
    ...
    def train(self, train_r, train_mask_r):
        for epoch in range(self._config['num_epoch']):
            ...
            # 构造一个DataLoader
            data_loader = Construct_DataLoader(train_r, train_mask_r, batch_size=self._config['batch_size'])
            # 训练一个轮次
